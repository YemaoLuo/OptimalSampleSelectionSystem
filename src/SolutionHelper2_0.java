...
public class SolutionHelper2_0 {
    ...
    public Map<List<Integer>, List<List<Integer>>> generateCoverListMap(List<List<Integer>> coverList, int s) {
        ...
        for (List<Integer> integers : coverList) {
            List<List<Integer>> subCoverList = generateCoverList(integers, s);
            for (List<Integer> integerList : subCoverList) {
                if (coverListMap.containsKey(integerList)) {
                    coverListMap.get(integerList).add(integers);
                } else {
                    List<List<Integer>> temp = new ArrayList<>();
                    temp.add(integers);
                    coverListMap.put(integerList, temp);
                }
            }
        }
        return coverListMap;
    ...
    public List<Integer> getCandidateResult(List<List<Integer>> possibleResults, Map<List<Integer>, List<List<Integer>>> coverListMap, int j) {
        List<Integer> candidateResult = new ArrayList<>();
        int max = Integer.MIN_VALUE;
        for (List<Integer> possibleResult : possibleResults) {
            int tempMax = 0;
            List<List<Integer>> subCoverList = generateCoverList(possibleResult, j);
            Set<List<Integer>> set = new HashSet<>();
            for (List<Integer> integers : subCoverList) {
                List<List<Integer>> lists = coverListMap.get(integers);
                if (lists == null) {
                    continue;
                }
    ...
    public List<List<Integer>> getResult(List<List<Integer>> possibleResults, Map<List<Integer>, List<List<Integer>>> coverListMap, int j) {
        List<List<Integer>> result = new ArrayList<>();
        ...
        while (!coverListMap.isEmpty()) {
            ...
            List<Integer> candidateResult = getCandidateResult(possibleResults, coverListMap, j);
